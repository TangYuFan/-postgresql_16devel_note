...
RemoveTempXlogFiles(void)
{
    ...
    printf("【log】xlog.c RemoveTempXlogFiles()\n");
				   ...
				   TimeLineID insertTLI)
{
    ...
    printf("【log】xlog.c RemoveOldXlogFiles() 删除无用的xlog记录 LSN=%d\n",segno);
			   ...
			   TimeLineID insertTLI)
{
    ...
    printf("【log】xlog.c RemoveXlogFile() 日志回收重用\n");
...
ValidateXLOGDirectoryStructure(void)
{
    ...
    printf("【log】xlog.c ValidateXLOGDirectoryStructure() 检测 pg_wal 和 archive_status。\n");
...
//    printf("【log】xlog.c ValidateXLOGDirectoryStructure() WAL directory:%s\n",XLOGDIR);
//    printf("【log】xlog.c ValidateXLOGDirectoryStructure() archive_status:%s\n",path);
...
void BootStrapXLOG(void)
{
    ...
    printf("【log】xlog.c BootStrapXLOG()\n");
...
void StartupXLOG(void)
{
    ...
    printf("【log】xlog.c StartupXLOG() 执行数据库恢复\n");
    InitWalRecovery
	XLogCtlInsert *Insert;
	CheckPoint	checkPoint;
	bool		wasShutdown;
    ...
    printf("【log】xlog.c StartupXLOG() wal_level:%d,state:%d,time:%s\n",ControlFile->wal_level,ControlFile->state,str_time(ControlFile->time));

    // 这里判断控制文件,根据控制文件状态判断是否需要从WAL中进行恢复
	 ...
	 */
    // 检测 pg_wal 和 pg_wal/archive_status 目录文件是否正常
    ...
    printf("【log】xlog.c StartupXLOG() 之前是否发生过崩溃 didCrash:%d\n",didCrash);


    // 分析 control-file 和 backup-label 来判断是否需要进行崩溃恢复或者存档恢复
    // 如果需要进行恢复,还分析出 wal 需要回放到某一个点
    // 主要工作是判断下面哪个更大:
    // ControlFile->minRecoveryPointTLI 控制文件记录的最小恢复点时间线
    // ControlFile->checkPointCopy.ThisTimeLineID 控制文件记录的上一次 checkpoint 的时间线
	InitWalRecovery(ControlFile, &wasShutdown,&haveBackupLabel, &haveTblspcMap);

    // 从控制文件读取的上一次检查点
	checkPoint = ControlFile->checkPointCopy;

    // 将检查点中的 ShmemVariableCache 恢复到内存中
	ShmemVariableCache->nextXid = checkPoint.nextXid;
	ShmemVariableCache->nextOid = checkPoint.nextOid;
	ShmemVariableCache->oidCount = 0;
	MultiXactSetNextMXact(checkPoint.nextMulti, checkPoint.nextMultiOffset);
    ...
    // PG 中存在两种高速缓存，这两个缓存在数据库中不是共享的，是每个进程独有的，通过共享消息队列来进行同步。
    // syscache主要用于缓存系统表元组；
    // relcache中包含所有访问过的表的模式信息（包含系统表）。
    // 这里清除 RelationCacheInitFile
	 ...
	 * required resources.
	 */
    // 加载 replication slots 所有复制槽从磁盘加载到内存
	...
	// 启动 clog 日志管理器 也就是提交日志记录每次commit的最终状态
	StartupCLOG();

	// 启动 multixact 日志管理器 multixact是记录一组事务ID
    // 因此同一个元组相关联的事务ID可能有多个，为了在加锁（行共享锁）的时候统一操作，uxdb将与该元组相关联的多个事务ID组合起来用一个MultiXactID代替来管理
	StartupMultiXact();
    ...
    printf("【log】前面得到标记后这里判断是否进入恢复模式 if(InRecovery)=%d:\n",InRecovery);
	...
	if (InRecovery)
	{
		 ...
		 */
        // 当前正在执行恢复,记录标记到控制文件,同时将检查点作为恢复的起点
		UpdateControlFile();
...
//    printf("【log】xlog.c RecoveryInProgress()\n");
...
GetInsertRecPtr(void)
{
    ...
    printf("【log】xlog.c GetInsertRecPtr() 获取当前插入位置 XLogCtl->LogwrtRqst.Write\n");
...
UpdateCheckPointDistanceEstimate(uint64 nbytes)
{
    ...
    printf("【log】xlog.c UpdateCheckPointDistanceEstimate() 估算两次 CheckPoint 之间的 xlog 日志量\n");
...
void CreateCheckPoint(int flags)
{
    ...
    printf("【log】xlog.c CreateCheckPoint() 开始执行检测点,flags=%d\n",flags);
...
CheckPointGuts(XLogRecPtr checkPointRedo, int flags)
{
    ...
    printf("【log】xlog.c CheckPointGuts() 将 shared memory 中脏数据 flush 到 disk\n");
...
CreateRestartPoint(int flags)
{
    ...
    printf("【log】xlog.c CreateRestartPoint()\n");
	...
	// 更新控制文件 redo 变量记录本次执行 CheckPoint 的 xlog 位置
	PriorRedoPtr = ControlFile->checkPointCopy.redo;
	...
	// 如果存在上一个检查点,估计到本次检查点的距离,也就是中间相差多少 xlog
	if (PriorRedoPtr != InvalidXLogRecPtr)
		UpdateCheckPointDistanceEstimate(RedoRecPtr - PriorRedoPtr);

	// 获取检查点的日志端号,并获取至少需要保留的 xlog 的 LSN
	XLByteToSeg(RedoRecPtr, _logSegNo, wal_segment_size);
    ...
    // 从至少需要保留的 LSN 往前减一,以此为分界之前的 xlog 可以进行删除
	_logSegNo--;
 ...
 */
void
do_pg_backup_start(const char *backupidstr, bool fast, List **tablespaces,BackupState *state, StringInfo tblspcmapfile)
{
