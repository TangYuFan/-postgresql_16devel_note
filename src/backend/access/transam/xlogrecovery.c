...
void InitWalRecovery(ControlFileData *ControlFile, bool *wasShutdown_ptr,bool *haveBackupLabel_ptr, bool *haveTblspcMap_ptr)
{
    ...
    printf("【log】xlogrecovery.c InitWalRecovery()\n");
    ...
    // 读取 backup_label 如果这个文件存在,优先从这里面读取检测点信息作为故障恢复的起点
    // backup_label 文件是 在 pg_start_backup 指令处理下生成的
    // 如果存在 backup_label 标志则优先使用 backup_label 中的检查点 而不是 pg_control 中标志的检查点
	if (read_backup_label(&CheckPointLoc, &CheckPointTLI, &backupEndRequired,&backupFromStandby))
	{
		...
		if (record != NULL)
		{
			...
			ereport(DEBUG1,(errmsg_internal("checkpoint record is at %X/%X",
				  ...
				  bool *backupEndRequired, bool *backupFromStandby)
{
    ...
    printf("【log】xlogrecovery.c read_backup_label() 读取 backup_label 如果这个文件存在,优先从这里面读取检测点信息作为故障恢复的起点\n");
...
PerformWalRecovery(void)
{
    ...
    printf("【log】xlogrecovery.c PerformWalRecovery() 执行 wal 恢复\n");
		   ...
		   bool fetching_ckpt, TimeLineID replayTLI)
{
    ...
    printf("【log】xlogrecover.c ReadRecord()\n");
			 ...
			 XLogRecPtr targetRecPtr, char *readBuf)
{
    ...
    printf("【log】xlogrecovery.c XLogPageRead()\n");
...
GetXLogReplayRecPtr(TimeLineID *replayTLI)
{
    ...
    printf("【log】xlogrecovery.c GetXLogReplayRecPtr() 获取 redo 最新位置\n");
