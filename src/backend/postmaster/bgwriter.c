...
void BackgroundWriterMain(void)
{
    ...
    printf("【log】bgwriter.c BackgroundWriterMain() \n");
	...
	for (;;)
	{
		...
		// 进行一次脏页写盘,并返回一个是否休眠变量
		can_hibernate = BgBufferSync(&wb_context);

		// 将统计数据发送到stats collector
		pgstat_report_bgwriter();

        // 判断是否是在 CheckPoint 之后的第一次操作
		 ...
		 * normal operation.
		 */
        //判断WaitLatch是由于哪种原因造成的返回
		rc = WaitLatch(MyLatch,WL_LATCH_SET | WL_TIMEOUT | WL_EXIT_ON_PM_DEATH,BgWriterDelay /* ms */ , WAIT_EVENT_BGWRITER_MAIN);
        ...
        //如果没有latch 事件，并且BgBufferSync没有落盘脏数据，函数就会睡眠一个比较长的时间。
