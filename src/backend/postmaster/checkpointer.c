...
{
    printf("【log】checkpointer.c CheckpointerMain()\n");
	...
	for (;;)
	{
		...
		if (do_checkpoint)
		{
            ...
            printf("【log】checkpointer.c 执行本次 checkpoint [%s]\n",str_time(now));
...
RequestCheckpoint(int flags)
{
    ...
    printf("【log】checkpointer.c RequestCheckpoint() 请求执行检测点,标记 CheckpointerShmem->ckpt_flags flags=%d\n",flags);
