...
PostmasterMain(int argc, char *argv[])
{
    ...
    printf("【log】postmaster.c PostmasterMain() 主程序入口\n");
	 ...
	 */
    printf("【log】postmaster.c 创建内存上下文 TopMemoryContext\n");
...
checkControlFile(void)
{
    ...
    printf("【log】postmaster.c checkControlFile() 检测 /global/pg_control 文件\n");
...
ServerLoop(void)
{
    ...
    printf("【log】postmaster.c ServerLoop() 开始 main loop, 调用 linux's epoll_wait(2) 处理客户端链接\n");
	...
	for (;;)
	{
		...
		nevents = WaitEventSetWait(pm_wait_set,DetermineSleepTime(),events,lengthof(events),0 /* postmaster posts no wait_events */ );
...
ConnCreate(int serverFd)
{
    ...
    printf("【log】postmaster.c ConnCreate() 客户端链接..\n");
...
//    printf("【log】postmaster.c InitProcessGlobals() 初始化pid文件、启动时间、随机数seeds\n");
...
BackendStartup(Port *port)
{
    ...
    printf("【log】postmaster.c BackendStartup() fork子进程处理当前链接,pid=%ld\n",pid);
...
BackendRun(Port *port)
{
    ...
    printf("【log】postmaster.c BackendRun() 客户端处理入口\n");
...
StartChildProcess(AuxProcType type)
{
    ...
    printf("【log】postmaster.c StartChildProcess() => 子进程启动,pid=%ld\n",pid);
...
maybe_start_bgworkers(void)
{
    ...
    printf("【log】postmaster.c maybe_start_bgworkers() \n");
